...
export default function ConfirmAirtime() {
  ...
  if (!state?.amount || !state?.network || !state?.phoneNumber) {
    ...
    <>
      <div className="w-[95%] md:w-[90%] mx-auto flex text-sm flex-col gap-5 bg-white-100 py-5">
        <div className="w-full p-5 bg-[#FBE9E7] flex flex-col gap-5 items-center justify-center font-medium">
          <h1>Total amount</h1>
          <h1 className="text-xl font-[600]">₦{state?.amount}</h1>
        </div>
        ...
        <div className="w-full space-y-5 rounded-xl border-t-8 p-5 border-green">
          ...
          <div className="flex justify-between items-center">
            ...
            <p className="text-right">
              {state?.network} airtime purchase for +234{state?.phoneNumber}
            </p>
          </div>
        </div>
        {/* confirm */}
      ...
      {modal && (
        <PinModal
          funcToCall={async () => {
            await PostDataService.topUpAirtime(
              state.amount,
              state.phoneNumber,
              token
            );
