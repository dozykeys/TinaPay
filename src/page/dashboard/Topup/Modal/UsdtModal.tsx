...
export default function UsdtModal({ data, setUsdtOpen }: any) {
  function closeModal() {
    setUsdtOpen(false);
  }
    ...
    <div className="fixed top-0 left-0 w-screen h-screen flex items-center justify-center backdrop-blur-sm">
      <div className="flex flex-col w-[90%] md:w-[50%] gap-10 bg-white mx-auto p-5 border-2 rounded-xl min-h-[50vh]">
        ...
        <div style={{width:200,height:"auto"}}>
          <QRCode value={data?.usdt_wallet_address} 
          size={100}
          ...
          <img src={copy} className="w-14" alt="" style={{cursor:"pointer"}} onClick={()=>CopyToClipboard(data.usdt_wallet_address)}/>
